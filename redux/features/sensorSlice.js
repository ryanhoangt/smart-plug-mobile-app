...
const isActionPending = (action) => action.type.endsWith('/pending')
const isActionRejected = (action) => action.type.endsWith('/rejected')
  ...
  async ({ token, sensor }, thunkAPI) => {
    try {
      ...
      mqttClient.subscribe(getAdafruitKey(newSensor.topic), (err) => {
        if (err)
          thunkAPI.rejectWithValue('Failed to subscribe ' + newSensor.topic)
